...
const Credits = ({ tvData, cast, crew, error }) => {
  ...
  return (
    <Fragment>
      <MetaWrapper
        title={error ? "Not Found - PlexFlix" : `${tvData.name} (${getReleaseYear(tvData?.first_air_date)}) - Credit - PlexFlix`}
        description={`${tvData.name} credit`}
        image={`https://image.tmdb.org/t/p/w780${tvData.backdrop_path}`}
        url={`${process.env.BUILD_URL}/tv/${getCleanTitle(tvData?.id, tvData?.name)}/credit`}
      />
      ...
      {error ? (
        ...
        <div className='relative mb-auto'>
          <DominantColor image={tvData?.poster_path} flip tint />
          <ModulesWrapper className='relative z-10'>
            <div className='text-center py-6'>
              <HeroInfoTitle className='mb-4'>
                <Link href={`/tv/${getCleanTitle(tvData?.id, tvData?.name)}`} passHref>
                  {tvData.name} ({getReleaseYear(tvData?.first_air_date)}) - Credits
                </Link>
              </HeroInfoTitle>
...
export const getServerSideProps = async (ctx) => {
  try {
    ...
    if (id !== `${expectedUrl}`) {
      return {
        redirect: {
          destination: `/tv/${expectedUrl}/credits`,
          permanent: false,
        },
      };
    }
      ...
      props: {
        ...
        cast: mergeEpisodeCount(
          data?.aggregate_credits?.cast
            ...
            .flat()
        ).sort((a, b) => b.popularity - a.popularity || b.episode_count - a.episode_count),
