...
const Crew = ({ movieData, crew, error }) => {
    ...
    <Fragment>
      <MetaWrapper
        title={
          error
            ? "Not Found - PlexFlix"
            : `${movieData?.title} (${getReleaseYear(
                movieData?.release_date
              )}) - Crew - plexflix`
        }
        description={error ? "Not Found" : `${movieData?.title} crew`}
...
export const getServerSideProps = async (ctx) => {
  try {
    ...
    const res = await fetch(
      apiEndpoints.movie.getMovieCredits({ id: movieId }),
      fetchOptions()
    );
    ...
    if (id !== `${expectedUrl}`) {
      return {
        redirect: {
          destination: `/movies/${expectedUrl}/crew`,
          permanent: false,
        },
      };
    }
