...
const SearchSuggestion = ({ data, type, ...props }) => (
  <Fragment>
    {type === "movie" && (
      <Link href={`/movies/${getCleanTitle(data.id, data.title)}`} passHref legacyBehavior>
        <Anchor {...props}>
          <SearchSlice>
    ...
    {type === "tv" && (
      <Link href={`/tv/${getCleanTitle(data.id, data.name)}`} passHref legacyBehavior>
        <Anchor {...props}>
          <SearchSlice>
            <h5 className='suggestion-title'>
              {data.name}{" "}
    ...
    {type === "person" && (
      <Link href={`/person/${getCleanTitle(data.id, data.name)}`} passHref legacyBehavior>
        <Anchor {...props}>
          <SearchSlice>
            <h5 className='suggestion-title'>
              {data.name}{" "}
